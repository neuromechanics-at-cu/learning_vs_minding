...
for nn = 1:length(ages);
    ...
    %% Load Data
    if ages(nn) == 'Y'
        load('../../Data/Model Fits/Model Accuracy Fits (Fig 3)/BestFitModel-FourPhases-Younger.mat')
        color = [0,128/255,0];
        titletext = 'Younger';
    elseif ages(nn) == 'O'
        load('../../Data/Model Fits/Model Accuracy Fits (Fig 3)/BestFitModel-FourPhases-Older.mat')
        color = [0,0,128/255];
        titletext = 'Older';
    else 
...
%     suptitle(titletext)
    ...
    for jj = 1:length(phaseNames)
        % Of Data
        eval(['fitGain(jj,1) = yData.',phaseNames{jj},'.estGainMean;'])
        eval(['fitGainSE(jj,1) = 1.96*yData.',phaseNames{jj},'.estGainSE;'])
