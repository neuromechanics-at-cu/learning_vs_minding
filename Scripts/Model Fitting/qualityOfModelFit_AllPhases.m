...
function [RMS] = qualityOfModelFit_AllPhases(X,subjData,subjDataChan,FitIndices,Params)
...
RMS_LN1 = (((subjData.LN1.PxSubjMean(end) - SOL_LN1.x(end,1))./subjData.LN1.PxSubjSE(end))^2)/1.96 + ...
      (((subjData.LN1.PySubjMean(end) - SOL_LN1.x(end,2))./subjData.LN1.PySubjSE(end))^2)/1.96 + ... 
      10*(((subjData.LN1.PxSubjMean(mp_ind_data_LN1) - SOL_LN1.x(mp_ind_model_LN1,1))./subjData.LN1.PxSubjSE(mp_ind_data_LN1))^2)/1.96 + ...
      8*(((subjData.LN1.PySubjMean(mp_ind_data_LN1) - SOL_LN1.x(mp_ind_model_LN1,2))./subjData.LN1.PxSubjSE(mp_ind_data_LN1))^2)/1.96 + ...
      28*(((subjDataChan.LN1.FxSubjMean(mp_ind_chandata_LN1) - SOL_LN1Chan.FxMax)./subjDataChan.LN1.FxSubjSE(mp_ind_chandata_LN1))^2)/1.96 + ...
      23*(((subjDataChan.LN1.estGainMean - SOL_LN1Chan.AdaptInd)./subjDataChan.LN1.estGainSE)^2)/1.96;
...
RMS_EF1 = (((subjData.EF1.PxSubjMean(end) - SOL_EF1.x(end,1))./subjData.EF1.PxSubjSE(end))^2)/1.96 + ...
      (((subjData.EF1.PySubjMean(end) - SOL_EF1.x(end,2))./subjData.EF1.PySubjSE(end))^2)/1.96 + ... 
      10*(((subjData.EF1.PxSubjMean(mp_ind_data_EF1) - SOL_EF1.x(mp_ind_model_EF1,1))./subjData.EF1.PxSubjSE(mp_ind_data_EF1))^2)/1.96 + ...
      8*(((subjData.EF1.PySubjMean(mp_ind_data_EF1) - SOL_EF1.x(mp_ind_model_EF1,2))./subjData.EF1.PxSubjSE(mp_ind_data_EF1))^2)/1.96 + ...
      28*(((subjDataChan.EF1.FxSubjMean(mp_ind_chandata_EF1) - SOL_EF1Chan.FxMax)./subjDataChan.EF1.FxSubjSE(mp_ind_chandata_EF1))^2)/1.96 + ...
      23*(((subjDataChan.EF1.estGainMean - SOL_EF1Chan.AdaptInd)./subjDataChan.EF1.estGainSE)^2)/1.96;
...
RMS_LF2 = (((subjData.LF2.PxSubjMean(end) - SOL_LF2.x(end,1))./subjData.LF2.PxSubjSE(end))^2)/1.96 + ...
      (((subjData.LF2.PySubjMean(end) - SOL_LF2.x(end,2))./subjData.LF2.PySubjSE(end))^2)/1.96 + ... 
      10*(((subjData.LF2.PxSubjMean(mp_ind_data_LF2) - SOL_LF2.x(mp_ind_model_LF2,1))./subjData.LF2.PxSubjSE(mp_ind_data_LF2))^2)/1.96 + ...
      8*(((subjData.LF2.PySubjMean(mp_ind_data_LF2) - SOL_LF2.x(mp_ind_model_LF2,2))./subjData.LF2.PxSubjSE(mp_ind_data_LF2))^2)/1.96 + ...
      28*(((subjDataChan.LF2.FxSubjMean(mp_ind_chandata_LF2) - SOL_LF2Chan.FxMax)./subjDataChan.LF2.FxSubjSE(mp_ind_chandata_LF2))^2)/1.96 + ...
      23*(((subjDataChan.LF2.estGainMean - SOL_LF2Chan.AdaptInd)./subjDataChan.LF2.estGainSE)^2)/1.96;
...
RMS_EN2 = (((subjData.EN2.PxSubjMean(end) - SOL_EN2.x(end,1))./subjData.EN2.PxSubjSE(end))^2)/1.96 + ...
      (((subjData.EN2.PySubjMean(end) - SOL_EN2.x(end,2))./subjData.EN2.PySubjSE(end))^2)/1.96 + ...
      10*(((subjData.EN2.PxSubjMean(mp_ind_data_EN2) - SOL_EN2.x(mp_ind_model_EN2,1))./subjData.EN2.PxSubjSE(mp_ind_data_EN2))^2)/1.96 + ...
      8*(((subjData.EN2.PySubjMean(mp_ind_data_EN2) - SOL_EN2.x(mp_ind_model_EN2,2))./subjData.EN2.PxSubjSE(mp_ind_data_EN2))^2)/1.96 + ...
      28*(((subjDataChan.EN2.FxSubjMean(mp_ind_chandata_EN2) - SOL_EN2Chan.FxMax)./subjDataChan.EN2.FxSubjSE(mp_ind_chandata_EN2))^2)/1.96 + ...
      23*(((subjDataChan.EN2.estGainMean - SOL_EN2Chan.AdaptInd)./subjDataChan.EN2.estGainSE)^2)/1.96;

% Add factor so that if it is outside 95% confidence interval
penalty = 0;
if (((subjData.LF2.PxSubjMean(mp_ind_data_LF2) - SOL_LF2.x(mp_ind_model_LF2,1))./subjData.LF2.PxSubjSE(mp_ind_data_LF2))^2)/1.96 > 1
    penalty = penalty + 1;
end
if (((subjDataChan.LF2.FxSubjMean(mp_ind_chandata_LF2) - SOL_LF2Chan.FxMax)./subjDataChan.LF2.FxSubjSE(mp_ind_chandata_LF2))^2)/1.96 > 1
    penalty = penalty + 1;
end
if (((subjDataChan.LF2.estGainMean - SOL_LF2Chan.AdaptInd)./subjDataChan.LF2.estGainSE)^2)/1.96 > 1
    penalty = penalty + 1;
end
if (((subjData.EN2.PxSubjMean(mp_ind_data_EN2) - SOL_EN2.x(mp_ind_model_EN2,1))./subjData.EN2.PxSubjSE(mp_ind_data_EN2))^2)/1.96 > 1
    penalty = penalty + 1;
end
if (((subjDataChan.EN2.FxSubjMean(mp_ind_chandata_EN2) - SOL_EN2Chan.FxMax)./subjDataChan.EN2.FxSubjSE(mp_ind_chandata_EN2))^2)/1.96 > 1
    penalty = penalty + 1;
end
if (((subjDataChan.EN2.estGainMean - SOL_EN2Chan.AdaptInd)./subjDataChan.EN2.estGainSE)^2)/1.96 > 1
    penalty = penalty + 1;
end
